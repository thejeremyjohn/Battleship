...
def get_guess():
    while True:
        ...
        if x not in range(wide) or y not in range(high):
            ...
            print 'Valid coordinates range from A0 to {}'.format(chr(wide+64) + str(high-1))
            continue
        return x + 1, y + 1
...
if __name__ == '__main__':
    ...
    backend_grid = []
    player_grid = []
    # put in letters for x-axis
    # backend_grid.append([' '] + map(str, range(wide)))
    # player_grid.append([' '] + map(str, range(wide)))
    backend_grid.append([' '] + [chr(i+65) for i in xrange(wide)])
    player_grid.append([' '] + [chr(i+65) for i in xrange(wide)])
